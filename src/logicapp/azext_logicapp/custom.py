...
from azure.cli.core.util import sdk_no_wait, shell_safe_json_parse, get_json_object, in_cloud_console, ConfiguredDefaultSetter
...
from ._constants import (FUNCTIONS_VERSION_TO_DEFAULT_RUNTIME_VERSION, FUNCTIONS_VERSION_TO_DEFAULT_NODE_VERSION,
                         KUBE_FUNCTION_APP_KIND, KUBE_FUNCTION_CONTAINER_APP_KIND, KUBE_CONTAINER_APP_KIND,
                         FUNCTIONS_VERSION_TO_SUPPORTED_RUNTIME_VERSIONS, NODE_VERSION_DEFAULT, NODE_EXACT_VERSION_DEFAULT,
                         DOTNET_RUNTIME_VERSION_TO_DOTNET_LINUX_FX_VERSION, KUBE_DEFAULT_SKU, APPSETTINGS_TO_MASK,
                         KUBE_ASP_KIND, KUBE_APP_KIND, KUBE_LOGIC_APP_KIND, KUBE_LOGIC_CONTAINER_APP_KIND,
                         LINUX_RUNTIMES, WINDOWS_RUNTIMES, MULTI_CONTAINER_TYPES, CONTAINER_APPSETTING_NAMES)
...
def create_kube_environment(cmd, client, name, resource_group_name, custom_location, static_ip,
    ...
    if not location:
        location = _get_location_from_resource_group(
            cmd.cli_ctx, parsed_custom_location['resource_group'])
...
def list_app_service_plans(cmd, resource_group_name=None):
    ...
    if resource_group_name is None:
        ...
        plans = list(client.app_service_plans.list_by_resource_group(
            resource_group_name))
    for plan in plans:
...
def _validate_asp_sku(app_service_environment, custom_location, sku):
    ...
    if sku.upper() in ['I1', 'I2', 'I3', 'I1V2', 'I2V2', 'I3V2']:
        ...
        if sku.upper() != 'K1':
            raise CLIError(
                "Only pricing tier 'K1' is allowed for this type of app service plan.")
    ...
    if custom_location:
        if not per_site_scaling:
            raise ArgumentUsageError(
                'Per Site Scaling must be true when using Custom Location. Please re-run with --per-site-scaling flag')
        if app_service_environment:
            raise ArgumentUsageError(
                'App Service Environment is not supported with using Custom Location')
        if hyper_v:
            raise ArgumentUsageError(
                'Hyper V is not supported with using Custom Location')
        if not is_linux:
            raise ArgumentUsageError(
                'Only Linux is supported with using Custom Location. Please re-run with --is-linux flag.')
...
def _get_kube_env_from_custom_location(cmd, custom_location):
    ...
    for kube in kube_envs:
        if kube.extended_location and kube.extended_location.custom_location:
            parsed_custom_location_2 = parse_resource_id(
                kube.extended_location.custom_location)
    ...
    if not kube_environment_id:
        raise ResourceNotFoundError(
            'Unable to find Kube Environment associated to the Custom Location')
...
def _get_custom_location_id_from_custom_location(cmd, custom_location_name):
    ...
    for kube in kube_envs:
        if kube.extended_location and kube.extended_location.custom_location:
            parsed_custom_location = parse_resource_id(
                kube.extended_location.custom_location)
...
def create_app_service_plan_inner(cmd, resource_group_name, name, is_linux, hyper_v, per_site_scaling=False, custom_location=None,
    ...
    if custom_location:
        kube_environment = _get_kube_env_from_custom_location(
            cmd, custom_location)

    if app_service_environment:
        if hyper_v:
            raise ArgumentUsageError(
                'Windows containers is not yet supported in app service environment')
        ase_id = _validate_app_service_environment_id(
            cmd.cli_ctx, app_service_environment, resource_group_name)
        ...
        if not ase_found:
            raise CLIError(
                "App service environment '{}' not found in subscription.".format(ase_id))
    else:  # Non-ASE
        ase_def = None

    extended_location_envelope = None
    if kube_environment and ase_def is None:
        kube_id = _resolve_kube_environment_id(
            cmd.cli_ctx, kube_environment, resource_group_name)
        ...
        if kube_name is not None and kube_rg is not None:
            kube_env = cf_kube_environments(
                cmd.cli_ctx).get(kube_rg, kube_name)
            extended_location_envelope = ExtendedLocationEnvelope(
                name=kube_env.extended_location.custom_location, type="CustomLocation")
            if kube_env is not None:
                location = kube_env.location
            else:
                raise CLIError(
                    "Kube Environment '{}' not found in subscription.".format(kube_id))
    else:
        kube_def = None
    ...
    sku_def = SkuDescription(tier=get_sku_name(
        sku), name=sku, capacity=number_of_workers)
...
def _validate_asp_and_custom_location_kube_envs_match(cmd, resource_group_name, custom_location, plan):
    ...
    if not plan_info:
        raise CLIError("The plan '{}' doesn't exist in the resource group '{}".format(
            plan, resource_group_name))

    plan_kube_env_id = ""
    custom_location_kube_env_id = _get_kube_env_from_custom_location(
        cmd, custom_location)
...
def _should_create_new_appservice_plan_for_k8se(cmd, name, custom_location, plan, resource_group_name):
    if custom_location and plan:
        ...
        if not existing_app_details:
            ...
            # existing app and kube environments match
            if _validate_asp_and_custom_location_kube_envs_match(cmd, resource_group_name, custom_location, existing_app_details.server_farm_id):
                return False
...
def create_webapp(cmd, resource_group_name, name, plan=None, runtime=None, custom_location=None, startup_file=None,  # pylint: disable=too-many-statements,too-many-branches
    ...
    if not plan and not custom_location:
        raise RequiredArgumentMissingError(
            "Either Plan or Custom Location must be specified")
    ...
    if not name_validation.name_available:
        ...
        logger.warning(
            "Webapp '%s' already exists. The command will use the existing app's settings.", name)
    ...
    if _should_create_new_plan:
        ...
        logger.warning(
            "Plan not specified. Creating Plan '%s' with sku '%s'", plan, KUBE_DEFAULT_SKU)
    ...
    if custom_location and plan:
        if not _validate_asp_and_custom_location_kube_envs_match(cmd, resource_group_name, custom_location, plan):
            raise ValidationError(
                "Custom location's kube environment and App Service Plan's kube environment don't match")
    ...
    if custom_location:
        ...
        _validate_asp_sku(app_service_environment=None,
                          custom_location=custom_location, sku=plan_info.sku.name)
    ...
    if custom_location or plan_info.kind.upper() == KUBE_ASP_KIND.upper() or (isinstance(plan_info.sku, SkuDescription) and plan_info.sku.name.upper() == KUBE_DEFAULT_SKU):
        ...
        if custom_location:  # if Custom Location provided, use that for Extended Location Envelope. Otherwise, get Custom Location from ASP
            custom_location_id = _get_custom_location_id_from_custom_location(
                cmd, custom_location)
            if custom_location_id:
                webapp_def.extended_location = ExtendedLocationEnvelope(
                    name=custom_location_id, type="CustomLocation")
        else:
    ...
    if is_kube:
        ...
        if deployment_container_image_name:
            ...
            if docker_registry_server_user is not None and docker_registry_server_password is not None:
                site_config.app_settings.append(NameValuePair(
                    name='DOCKER_REGISTRY_SERVER_USERNAME', value=docker_registry_server_user))
                site_config.app_settings.append(NameValuePair(
    ...
    if is_linux or is_kube:
        ...
        if runtime:
            ...
            encoded_config_file = _get_linux_multicontainer_encoded_config_from_file(
                multicontainer_config_file)
            site_config.linux_fx_version = _format_fx_version(
                encoded_config_file, multicontainer_config_type)

    elif plan_info.is_xenon:  # windows container webapp
        if deployment_container_image_name:
            site_config.windows_fx_version = _format_fx_version(
                deployment_container_image_name)
    ...
    poller = client.web_apps.create_or_update(
        resource_group_name, name, webapp_def)
    ...
    if current_stack:
        _update_webapp_current_stack_property_if_needed(
            cmd, resource_group_name, name, current_stack)
...
def webapp_up(cmd, name, resource_group_name=None, plan=None, location=None, sku=None, dryrun=False, logs=False,  # pylint: disable=too-many-statements,
    ...
    if environment is not None:
        if resource_group_name is None:
            raise CLIError("Must provide a resource group where Kube Environment '{}' exists".format(
                kube_environment))
        kube_id = _resolve_kube_environment_id(
            cmd.cli_ctx, environment, resource_group_name)
        is_kube = True
        kube_def = KubeEnvironmentProfile(id=kube_id)
        parsed_id = parse_resource_id(kube_id)
        kube_name = parsed_id.get("name")
    ...
    # detect the version
    data = get_runtime_version_details(
        lang_details.get('file_loc'), language, is_kube)
    ...
    if not _create_new_app:  # App exists
        ...
        logger.warning(
            "Webapp %s already exists. The command will deploy contents to the existing app.", name)
        app_details = get_app_details(cmd, name, resource_group_name)
        if app_details is None:
        ...
        sku = plan_info.sku.name if isinstance(
            plan_info, AppServicePlan) else 'Free'
        ...
        if is_kube:
            ...
            _create_new_rg = False
            _is_linux = False
            plan = get_kube_plan_to_use(
                cmd, environment, loc, sku, rg_name, _create_new_rg, plan)
            sku_name = kube_sku
        else:
            sku = get_sku_to_use(src_dir, html, sku)
            loc = set_location(cmd, sku, location)
            rg_name = get_rg_to_use(
                cmd, user, loc, os_name, resource_group_name)
            _is_linux = os_name.lower() == 'linux'
            _create_new_rg = should_create_new_rg(cmd, rg_name, _is_linux)
            sku_name = get_sku_name(sku)
            plan = get_plan_to_use(
                cmd, user, os_name, loc, sku_name, rg_name, _create_new_rg, plan)
    dry_run_str = r""" {
                "name" : "%s",
                "appserviceplan" : "%s",
                "resourcegroup" : "%s",
    ...
    if is_kube:
        logger.warning(
            "Creating AppServicePlan '%s' with 1 instance count on kube cluster '%s' ...", plan, environment)
    else:
        logger.warning("Creating AppServicePlan '%s' ...", plan)
    ...
    if _create_new_app:
        ...
        if not is_kube:
            ...
            logger.warning(
                "Successfully App '%s' on Kube Cluster '%s'...", plan, environment)
        ...
        if os_name.lower() == 'linux' and site_config.linux_fx_version != runtime_version:
            ...
            update_site_configs(cmd, rg_name, name,
                                linux_fx_version=runtime_version)
            ...
            update_site_configs(cmd, rg_name, name,
                                windows_fx_version=runtime_version)
...
def show_webapp(cmd, resource_group_name, name, slot=None, app_instance=None):
    ...
    webapp.site_config = _generic_site_operation(
        cmd.cli_ctx, resource_group_name, name, 'get_configuration', slot)
...
def create_logicapp(cmd, resource_group_name, name, storage_account, plan=None, custom_location=None,
    ...
    if not plan and not consumption_plan_location and not custom_location:
        raise RequiredArgumentMissingError(
            "Either Plan, Consumption Plan, or Custom Location must be specified")

    if consumption_plan_location and custom_location:
        raise MutuallyExclusiveArgumentError(
            "Consumption Plan and Custom Location cannot be used together")

    if consumption_plan_location and plan:
        raise MutuallyExclusiveArgumentError(
            "Consumption Plan and Plan cannot be used together")

    SiteConfig, Site, NameValuePair, ExtendedLocationEnvelope = cmd.get_models(
        'SiteConfig', 'Site', 'NameValuePair', 'ExtendedLocationEnvelope')
    ...
    if consumption_plan_location:
        ...
        location = next((l for l in locations if l['name'].lower(
    ...
    con_string = _validate_and_get_connection_string(
        cmd.cli_ctx, resource_group_name, storage_account)
    ...
    if is_kube:
        if custom_location:  # if Custom Location provided, use that for Extended Location Envelope. Otherwise, get Custom Location from ASP
            custom_location_id = _get_custom_location_id_from_custom_location(
                cmd, custom_location)
            if custom_location_id:
                logicapp_def.extended_location = ExtendedLocationEnvelope(
                    name=custom_location_id, type="CustomLocation")
        else:
            logicapp_def.extended_location = plan_info.extended_location
        ...
        if deployment_container_image_name:
            logicapp_def.kind = KUBE_LOGIC_CONTAINER_APP_KIND
            ...
            site_config.linux_fx_version = _get_linux_fx_kube_functionapp(
...
def create_function(cmd, resource_group_name, name, storage_account, plan=None, custom_location=None,
    ...
    if runtime_version is not None:
        ...
        allowed_versions = FUNCTIONS_VERSION_TO_SUPPORTED_RUNTIME_VERSIONS[
            functions_version][runtime]
    ...
    if is_kube:
        if custom_location:  # if Custom Location provided, use that for Extended Location Envelope. Otherwise, get Custom Location from ASP
            custom_location_id = _get_custom_location_id_from_custom_location(
                cmd, custom_location)
            if custom_location_id:
                functionapp_def.extended_location = ExtendedLocationEnvelope(
                    name=custom_location_id, type="CustomLocation")
        else:
    ...
    site_config.app_settings.append(NameValuePair(
        name='AzureWebJobsStorage', value=con_string))
    site_config.app_settings.append(NameValuePair(
        name='AzureWebJobsDashboard', value=con_string))
    site_config.app_settings.append(NameValuePair(name='WEBSITE_NODE_DEFAULT_VERSION',
    ...
    poller = client.web_apps.create_or_update(
        resource_group_name, name, functionapp_def)
...
def update_container_settings(cmd, resource_group_name, name, docker_registry_server_url=None,
    ...
    if docker_registry_server_url is not None:
        settings.append('DOCKER_REGISTRY_SERVER_URL=' +
    ...
    if (not docker_registry_server_user and not docker_registry_server_password and
        ...
        try:
            docker_registry_server_user, docker_registry_server_password = _get_acr_cred(
                cmd.cli_ctx, registry_name)
        except Exception as ex:  # pylint: disable=broad-except
            # consider throw if needed
            logger.warning(
                "Retrieving credentials failed with an exception:'%s'", ex)

    if docker_registry_server_user is not None:
        settings.append('DOCKER_REGISTRY_SERVER_USERNAME=' +
                        docker_registry_server_user)
    if docker_registry_server_password is not None:
        settings.append('DOCKER_REGISTRY_SERVER_PASSWORD=' +
                        docker_registry_server_password)
    if docker_custom_image_name is not None:
        _add_fx_version(cmd, resource_group_name, name,
                        docker_custom_image_name, slot)
    if websites_enable_app_service_storage:
        settings.append('WEBSITES_ENABLE_APP_SERVICE_STORAGE=' +
    ...
    if multicontainer_config_file and multicontainer_config_type:
        ...
        linux_fx_version = _format_fx_version(
            encoded_config_file, multicontainer_config_type)
        update_site_configs(cmd, resource_group_name, name,
                            linux_fx_version=linux_fx_version, slot=slot)
    elif multicontainer_config_file or multicontainer_config_type:
        logger.warning(
            'Must change both settings --multicontainer-config-file FILE --multicontainer-config-type TYPE')
...
def try_create_application_insights(cmd, functionapp):
    ...
    app_insights_client = get_mgmt_service_client(
        cmd.cli_ctx, ApplicationInsightsManagementClient)
    ai_properties = {
        "name": ai_name,
    ...
    appinsights = app_insights_client.components.create_or_update(
        ai_resource_group_name, ai_name, ai_properties)
...
def update_site_configs(cmd, resource_group_name, name, slot=None, number_of_workers=None, linux_fx_version=None,
    ...
    if number_of_workers is not None:
        number_of_workers = validate_range_of_int_flag(
    ...
    args, _, _, values = inspect.getargvalues(
        frame)  # pylint: disable=deprecated-method
...
def config_source_control(cmd, resource_group_name, name, repo_url, repository_type='git', branch=None,  # pylint: disable=too-many-locals
    ...
    if cd_project_url:
        ...
        webapp_list = None if test is None else list_webapp(
            resource_group_name)
    ...
    if git_token:
        sc = SourceControl(location=location,
                           source_control_name='GitHub', token=git_token)
...
def list_publish_profiles(cmd, resource_group_name, name, slot=None, xml=False):
    ...
    if not xml:
        profiles = xmltodict.parse(full_xml, xml_attribs=True)[
...
def _set_remote_or_local_git(cmd, webapp, resource_group_name, name, deployment_source_url=None,
    ...
    if deployment_source_url:
        ...
        try:
            ...
            ex = ex_handler_factory(no_throw=True)(ex)
            logger.warning(
                "Link to git repository failed due to error '%s'", ex)

    if deployment_local_git:
        local_git_info = enable_local_git(cmd, resource_group_name, name)
        logger.warning("Local git is configured with url of '%s'",
                       local_git_info['url'])
...
def _add_fx_version(cmd, resource_group_name, name, custom_image_name, slot=None):
    ...
    if not web_app:
        raise CLIError("'{}' app doesn't exist in resource group {}".format(
            name, resource_group_name))
    linux_fx = fx_version if (
        web_app.reserved or not web_app.is_xenon) else None
...
def _get_app_settings_from_scm(cmd, resource_group_name, name, slot=None):
    ...
    username, password = _get_site_credential(
        cmd.cli_ctx, resource_group_name, name, slot)
    headers = {
...
def validate_app_settings_in_scm(cmd, resource_group_name, name, slot=None,
    ...
    scm_settings = _get_app_settings_from_scm(
...
def remove_remote_build_app_settings(cmd, resource_group_name, name, slot):
    ...
    if app_settings_should_contain:
        ...
        if retries < 0:
            logger.warning(
                "App settings may not be propagated to the SCM site")
...
def enable_zip_deploy_functionapp(cmd, resource_group_name, name, src, build_remote=False, timeout=None, slot=None):
    ...
    # We need to retry getting the plan because sometimes if the plan is created as part of logic app,
    ...
    if build_remote and not app.reserved:
        raise CLIError('Remote build is only available on Linux logic apps')
...
def enable_zip_deploy(cmd, resource_group_name, name, src, timeout=None, slot=None, is_kube=False):
    ...
    user_name, password = _get_site_credential(
        cmd.cli_ctx, resource_group_name, name, slot)
    ...
    try:
        ...
        raise CLIError('Failed to fetch scm url for logic app')
    ...
    with open(os.path.realpath(os.path.expanduser(src)), 'rb') as fs:
        ...
        logger.warning(
            "Deployment endpoint responded with status code %d", res.status_code)

        if is_kube and res.status_code != 202 and res.status_code != 409:
            logger.warning(
                'Something went wrong. It may take a few seconds for a new deployment to reflect on kube cluster. Retrying deployment...')
...
def _check_zip_deployment_status(cmd, rg_name, name, deployment_status_url, authorization, timeout=None):
    ...
    while num_trials < total_trials:
        ...
        try:
            ...
            logger.warning(
                "Deployment status endpoint %s returns malformed data. Retrying...", deployment_status_url)
            res_dict = {}
        finally:
...
def _fill_ftp_publishing_url(cmd, webapp, resource_group_name, name, slot=None):
    ...
    try:
        url = next((p['publishUrl']
                   for p in profiles if p['publishMethod'] == 'FTP'), None)
